...
export default function MoveIn() {
  ...
  const [moveOutForm, setMoveOutForm] = useState({ reason: '', preferredDate: '' });
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {moveIn.status !== 'completed' && (
        <div className="bg-white rounded-2xl border border-red-100 shadow-sm p-6">
          <h2 className="font-bold text-gray-900 text-lg mb-1">🚪 Request Move-Out</h2>
          <p className="text-sm text-gray-400 mb-4">Submit a move-out request to the admin. You'll be notified once it's approved.</p>

          {moveIn.moveOut?.status === 'requested' && (
            <div className="bg-amber-50 border border-amber-200 text-amber-700 rounded-xl px-4 py-3 text-sm font-medium mb-4">
              ⏳ Move-out request submitted — awaiting admin approval
            </div>
          )}
          {moveIn.moveOut?.status === 'approved' && (
            <div className="bg-green-50 border border-green-200 text-green-700 rounded-xl px-4 py-3 text-sm font-medium mb-4">
              ✅ Move-out approved! You can vacate on {moveIn.moveOut?.preferredDate ? new Date(moveIn.moveOut.preferredDate).toLocaleDateString('en-IN') : 'agreed date'}.
            </div>
          )}

          {(!moveIn.moveOut?.status || moveIn.moveOut?.status === 'none') && (
            <form onSubmit={async (e) => {
              e.preventDefault();
              setMoveOutLoading(true);
              try {
                const res = await api.post(`/api/movein/${id}/moveout-request`, moveOutForm);
                setMoveIn(res.data.moveIn);
                setMsg('✅ Move-out request submitted!');
              } catch (err) { setError(err.response?.data?.message || 'Failed'); }
              finally { setMoveOutLoading(false); }
            }} className="space-y-3">
              <div>
                <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Preferred Move-Out Date *</label>
                <input required type="date" value={moveOutForm.preferredDate} onChange={e => setMoveOutForm(f => ({ ...f, preferredDate: e.target.value }))}
                  min={new Date().toISOString().split('T')[0]}
                  className="w-full mt-1 border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-400" />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Reason *</label>
                <textarea required rows={3} value={moveOutForm.reason} onChange={e => setMoveOutForm(f => ({ ...f, reason: e.target.value }))}
                  placeholder="Please provide your reason for moving out..."
                  className="w-full mt-1 border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-400 resize-none" />
              </div>
              <button type="submit" disabled={moveOutLoading}
                className="bg-red-500 text-white px-6 py-2.5 rounded-xl text-sm font-semibold hover:bg-red-600 disabled:opacity-50 transition-colors">
                {moveOutLoading ? 'Submitting...' : 'Submit Move-Out Request'}
              </button>
            </form>
          )}
        </div>
